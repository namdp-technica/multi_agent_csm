...
class CosmoFlowAgent(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        try:
            ...
            all_images = await self._step2_parallel_search_sync(
                ctx, search_tasks, user_input
            )

            # Step 3: VLM Agents phân tích ảnh
            logger.info(f"[{self.name}] Step 3: VLM Agents analyzing images...")
            vlm_results = await self._step3_vlm_analysis_sync(
                ctx, user_input, all_images
            )

            # Step 4: Aggregator tổng hợp kết quả
            logger.info(f"[{self.name}] Step 4: Aggregating final results...")
            final_answer = await self._step4_aggregate_results_sync(
                ctx, user_input, vlm_results
            )
    ...
    async def _step1_main_agent_analysis_sync(
        self, ctx: InvocationContext, user_query: str
    ) -> List[Dict[str, str]]:
        """
        Step 1: Main Agent phân tích câu hỏi và chia thành sub-tasks
        """
        logger.info(
            f"[{self.name}] Main Agent analyzing and breaking down the query..."
        )
        ...
        try:
            ...
            async for event in self.main_agent.run_async(ctx):
                event_count += 1
                logger.info(
                    f"[{self.name}] Main Agent Event #{event_count}: {event.model_dump_json(indent=2, exclude_none=True)}"
                )
                ...
                if event_count >= max_main_events:
                    logger.warning(
                        f"[{self.name}] Main Agent: Reached max events limit"
                    )
                    break
    ...
    async def _step2_parallel_search_sync(
        self,
        ...
        """
        logger.info(
            f"[{self.name}] Running {len(self.search_agents)} Search Agents in parallel..."
        )
    ...
    async def _step2_parallel_search_sync(
        ...
        async def run_search_agent(agent, task, agent_idx):
            try:
                ...
                async for event in agent.run_async(ctx):
                    event_count += 1
                    logger.info(
                        f"[{self.name}] SearchAgent{agent_idx} Event #{event_count}: {event.model_dump_json(indent=2, exclude_none=True)}"
                    )
                        ...
                        for part in event.content.parts:
                            if (
                                ...
                                if (
                                    ...
                                    logger.info(
                                        f"[{self.name}] SearchAgent{agent_idx}: Found {len(search_results)} images in function response"
                                    )
                    ...
                    if event_count >= max_events:
                        logger.warning(
                            f"[{self.name}] SearchAgent{agent_idx}: Reached max events limit ({max_events})"
                        )
                        break

                logger.info(
                    f"[{self.name}] SearchAgent{agent_idx}: Found {len(search_results)} images total"
                )
        ...
        search_tasks_limited = search_tasks[
            : len(self.search_agents)
        ]  # Giới hạn số task
        search_coroutines = [
    ...
    async def _step3_vlm_analysis_sync(
        self, ctx: InvocationContext, user_query: str, all_images: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        Step 3: VLM Agents phân tích ảnh song song
        """
        logger.info(
            f"[{self.name}] VLM Agents processing {len(all_images)} images in parallel..."
        )
...
Please analyze this image and answer the user's question."""
    ...
    async def _step4_aggregate_results_sync(
        self, ctx: InvocationContext, user_query: str, vlm_results: List[Dict[str, Any]]
    ) -> str:
        """
        ...
        vlm_results_str = "\n".join(
            [
                f"Result from VLM{i} : {vlm_result['response']} (with reference {vlm_result['image_id']} of relevant score {vlm_result['relevance_score']}) \n"
                for i, vlm_result in enumerate(vlm_results)
            ]
        )
        try:
...
Please provide a comprehensive answer based on the VLM analysis results. Print out reference doc id and relevant score. If the vlm agent do not know the answer ignore that specific vlm agent."""
            ...
            async for event in events:
                event_count += 1
                logger.info(
                    f"[{self.name}] Aggregator Event #{event_count}: {event.model_dump_json(indent=2, exclude_none=True)}"
                )
                if hasattr(event, "content") and event.content and event.content.parts:
                    if event.content.parts[0].text:
                ...
                if event_count >= max_aggregator_events:
                    logger.warning(
                        f"[{self.name}] Aggregator: Reached max events limit"
                    )
                    break

            logger.info(
                f"[{self.name}] Final answer generated ({len(final_answer)} chars)"
            )
            return final_answer
    ...
    def _fallback_aggregation(
        self, user_query: str, vlm_results: List[Dict[str, Any]]
    ) -> str:
        """
